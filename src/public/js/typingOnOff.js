...
$(document).ready(function(){
    ...
    socket.on("response-user-is-typing",function(response){
        let messaageTyping = `<div class="bubble you bubble-typing-gif">
        ...
        if(response.currentGroupId){
            if(response.currentUserId !== $("#dropdown-navbar-user").data("uid")){
                ...
                $(`.chat[data-chat = ${response.currentGroupId}]`).append(messaageTyping);
            ...
            $(`.chat[data-chat = ${response.currentUserId}]`).append(messaageTyping);
            nineScrollRight(response.currentUserId);
        }
    });

    // lăng nghe typing off
